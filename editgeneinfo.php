...
?>
	...
	?>
			<form role="form" action="editgeneinfo.php" method="post">
	...
	<?php
		...
		$_SESSION['username-post-data'] = $getUser;
		...
		<form role="form" action="editgeneinfo.php" method="post">
			<div class="form-group">
				...
				<br/>
				<select name="genename" onChange="this.form.submit()">
				<?php 		
				foreach ($geneinfoarrfull as $key => $geneinfo) {
					if ($geneinfo['genename'] == $_POST['genename']) {
				?>
					<option value="<?php echo $geneinfo['genename'] ?>" selected="selected"><?php echo $geneinfo['genename'] ?></option>
				<?php
					} else {
				?>
					<option value="<?php echo $geneinfo['genename'] ?>"><?php echo $geneinfo['genename'] ?></option>
				<?php
					}
				}
				?>
		...
		<form role="form" action="savegeneinfo.php" method="post">
			...
			<div class="form-group">
				<label for="genename">Gene Name</label>
				<input name="genename" class="form-control" type="text" value="<?php echo $geneinfoarr['genename'];?>"/>
			</div>
			<div class="form-group">
				<label for="genename2">Gene Name2</label>
				<input name="genename2" class="form-control" type="hidden" value="<?php echo $geneinfoarr['genename'];?>"/>
			</div>

			<div class="form-group">
				<label for="systematicname">Systematic Name</label>
				<input name="systematicname" class="form-control" type="text" value="<?php echo $geneinfoarr['systematicname'];?>" /> 
			</div>
			<div class="form-group">
				<label for="size">Size</label>
				<input name="size" class="form-control" type="text" value="<?php echo $geneinfoarr['size'];?>" />
			</div>
			<div class="form-group">
				<label for="info">Function</label>
				<textarea name="info" class="form-control"><?php echo $geneinfoarr['info'];?></textarea>
			</div>
			<div class="form-group">
			...
			<button type="submit" name="updater" value="updated" class="btn btn-primary">Update</button>
			<button type="submit" name="updater" value="cancel" formaction="geneinfo.php" class="btn btn-primary">Cancel</button>
			<button type="submit" name="updaterd" value="delete" formaction="savegeneinfo.php" formmethod="POST" class="btn btn-primary">Delete Gene</button>
